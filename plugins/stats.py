...
@bot_client.on(events.NewMessage(pattern=r'^/get(?:\s+(.*))?$'))
async def get_all_users_handler(event):
    # only private
    if not await is_private_chat(event):
    ...
    total = len(user_ids)
    if total == 0:
        await event.respond("No users found in DB.")
        return

    # preview mode: /get preview
    if arg in ("preview", "p"):
        preview_count = min(50, total)
        preview_list = "\n".join(str(x) for x in user_ids[:preview_count])
        await event.respond(
            f"✅ Total users: **{total}**\n\n"
    ...
    # send file mode (default)
    filename = "user_ids.txt"
    with open(filename, "w", encoding="utf-8") as f:
        f.write("\n".join(str(x) for x in user_ids))

    try:
        await event.respond(f"✅ Total users in DB: **{total}**\n📄 Sending file…")
        await bot_client.send_file(
            event.chat_id,
            filename,
            caption=f"✅ Exported {total} user IDs"
        )
    finally:
        try:
            os.remove(filename)
