...
def is_telegram_link(url: str) -> bool:
    u = (url or "").lower()
    return ("t.me/" in u) or ("telegram.me/" in u) or ("telegram.org/" in u)

def telegram_block_text() -> str:
    return (
        "❌ **Telegram links are not supported in /dl or /adl (yt-dlp).**\n\n"
        "✅ For Telegram private/public channel/group downloads use:\n"
        "• `/login` (required for private)\n"
        "• `/single <t.me link>`\n"
        "• `/batch <start link>`\n\n"
        "**Reason:** Telegram private media works only via user session (MTProto), not yt-dlp."
    )

def resolve_cookies(cookies_env_var: Optional[str]):
    """
    Keeps compatibility:
    - Your handlers pass 'YT_COOKIES'/'INSTA_COOKIES' currently.
    - This resolver returns actual cookie string from config or env.
    """
    if not cookies_env_var:
        return None

    # If passed literal cookie content (rare), accept it
    if "Netscape" in cookies_env_var or "youtube" in cookies_env_var.lower() or "instagram" in cookies_env_var.lower():
        return cookies_env_var
...
async def process_audio(client, event, url, cookies_env_var=None):
    # BLOCK Telegram links here too (safety)
    if is_telegram_link(url):
        await event.reply(telegram_block_text())
        return

    cookies = resolve_cookies(cookies_env_var)
    ...
    random_filename = f"@eurnyme_{event.sender_id}"
    ...
    try:
        ...
        if os.path.exists(download_path):
            def edit_metadata():
                ...
                audio_file.tags["TPE1"] = TPE1(encoding=3, text="@azsaverestricted1bot")
                audio_file.tags["COMM"] = COMM(encoding=3, lang="eng", desc="Comment", text="Processed by @azsaverestricted1bot")
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    # BLOCK Telegram links (MOST IMPORTANT)
    if is_telegram_link(url):
        await event.reply(telegram_block_text())
        return
